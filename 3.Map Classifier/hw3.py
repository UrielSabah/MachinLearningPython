...
class NaiveNormalClassDistribution():
    def __init__(self, dataset, class_value):
        ...
        """
        # Get just the data with tie right class
...
class DiscreteNBClassDistribution():
    def __init__(self, dataset, class_value):
        ...
        self.A = np.array(dataset[dataset[:, -1] == class_value][:, 0:-1])
        self.n_i = self.A.shape[0]

    def get_prior(self):
        """
        Returns the prior porbability of the class according to the dataset distribution.
        """
        return self.n_i / self.dataset.shape[0]
    ...
    def get_instance_likelihood(self, x):
        ...
        for i in range(self.dataset.shape[1] - 1):
            nij = (self.A[:, i] == x[i]).sum()
            vj = np.unique(self.A[:, i])
            if (nij != 0):
                p = (nij + 1) / (self.n_i + len(vj))
            else:
                p = EPSILLON
