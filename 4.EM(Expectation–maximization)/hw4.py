...
def init(points_list, k):
    ...
    ###########################################################################
    np.random.seed(1)
    w = np.array([1 / k ])  #first guess
    mu = np.random.random(k) * 35
    sigma = np.random.random(k) * 10
...
def expectation(points_list, mu, sigma, w):
    ...
    likelihood = w * ((1 / np.sqrt(2 * np.pi * np.square(sigma))) *
                      np.exp( -1 * (np.square(points_list.T - mu)) / (2 * np.square(sigma))))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def maximization(points_list, ranks):
   ...
   #:param data: the complete data
    ...
    ###########################################################################
    points_list = np.array([points_list])
    w_new = ranks.sum(axis=0) / len(ranks)

    mu_new = (points_list.dot(ranks).sum(axis=0) / w_new / len(ranks))

    sigma_new = np.array([])
    for k in range(len(mu_new)):
        sum = 0
        for i in range(len(ranks)):
            sum += ranks[i][k] * np.square(points_list[0][i] - mu_new[k])
        sigma_new = np.append(sigma_new, np.sqrt(sum / w_new[k] / len(ranks)))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def calc_max_delta(old_param, new_param):
    ...
    ###########################################################################
    max_delta = (np.abs(old_param - new_param)).max()
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def expectation_maximization(points_list, k, max_iter, epsilon):
    ...
    ####################################################################################
    w, mu, sigma = init(points_list, k)                    #### initiating
    ####################################################################################
    ...
    while delta > epsilon and iter_num <= max_iter:
        ...
        # M step
        ####################################################################################
        ranks = likelihood / np.array([likelihood_sum]).T   # TODO: compute ranks array using the likelihood array
        ####################################################################################
        ...
        ####################################################################################
        w, mu, sigma = w_new, mu_new, sigma_new               #### actualizing
        ####################################################################################
        if iter_num % 10 == 0:
